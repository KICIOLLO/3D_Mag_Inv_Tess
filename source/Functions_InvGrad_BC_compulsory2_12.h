...
// cal_Magnetic_mag_Farq_new 
double cal_Magnetic_mag_Farq_compulsory_noneffsto(int N_data, int N_mod, double miu,double * deltaT,double * G,
	...
	double * RdHax,double * RdHay,double * RdZa,double * RdTa,double * Rd_2,
	...
	double * tran_r22,double * tran_r23,double * tran_r31,double * tran_r32,double * tran_r33)
{
	...
	for(i=0;i<N_mod;i++)
		// temp_x[i] = miu*m_alpha_x[i]*temp_x[i];
	...
	for(i=0;i<N_mod;i++)
		// temp_y[i] = miu*m_alpha_y[i]*temp_y[i];
	...
	if(paraInvOrNot[0] != 0){
		...
		// SaveFile("4.2.1_4.dat",N_mod,temp_d);
		mkl_free(temp_d1);
	}
	else{
		temp_d = (double *)mkl_malloc(N_mod*sizeof( double ), 64);
	...
	if(paraInvOrNot[1] != 0){
		...
		//SaveFile("4.2.1_3.dat",N_data,temp_d1);
...
// 	for(i=0;i<N_mod;i++)
// 		temp_lbm_B[i] = lamoda0*((1/(m_0_n[i]-m_min_n[i])) + 1/(m_0_n[i]-m_max_n[i]));
// // 	delete[] temp_d1;
// 	// printf("temp_d\n");
	...
	// 这一段跟后面的长段，是相同的，只是如果X0不是默认的全部是0，则需要用下面的这一段来进行处理
...
// 		delete(temp_z1);
	...
	double * m_result_temp;
	m_result_temp = (double *)mkl_malloc( N_mod*sizeof( double ), 64 );
	#pragma omp parallel for shared(N_mod,m_result_temp,X1) private(i)
	for(i=0;i<N_mod;i++){
		m_result_temp[i] = m_0_n[i] + X1[i];
	}
	for(i=0;i<N_mod;i++){
		if(m_result_temp[i]>m_max_n[i]){
			m_result[i] = m_max_n[i];
		}
		else if(m_result_temp[i]<m_min_n[i]){
			m_result[i] = m_min_n[i];
		}
		else{
			m_result[i] = m_result_temp[i];
		}
	}
...
int cal_datamisfit_objmodfun_compulsory_noneffsto(int N_data, int N_mod,double miu,double * deltaT,
...
double InversionMagSus_compulsory_noneffsto(TESSEROID *model, OBSMAGPOS *obs_pos_mag, int N_data, int N_mod, 
	...
	int * index_lon, int * index_lat, int * index_depth, unsigned long long N_data_ULL, unsigned long long N_mod_ULL)
{
	...
	// Construct the Ws Wx Wy Wz matrix: Start(To Line:366)
	...
	if(para_deepweight_type == 1){
		...
		for(i_tmp=0ULL;i_tmp<N_mod_ULL;i_tmp++){
			...
			// W_deepw[i] = sqrt(sqrt(temp_wdeepsum));
	...
	// Because Jcobi matrix is same as the G matrix in the linear problems
	// we didn't construct another matrix for Jcobi
	int internum;
	...
	cal_Magnetic_mag_Farq_compulsory_noneffsto(N_data,N_mod,miu,obs_deltaT,G_T,Wd,
	...
	cal_datamisfit_objmodfun_compulsory_noneffsto(N_data,N_mod,miu,obs_deltaT,G_T,Wd,
		obs_deltaTHax,G_THax,WdHax,obs_deltaTHay,G_THay,WdHay,obs_deltaTZa,G_TZa,WdZa,obs_deltaTTa,G_TTa,WdTa,
		obs_deltaT_2, G_T_2, Wd_2, geomag_ref_x, geomag_ref_y, geomag_ref_z,
		paraInvOrNot,Ws,Wx,
		columns_x,rowIndex_x,Wy,columns_y,rowIndex_y,Wz,columns_z,rowIndex_z,W_deepw,
	...
	cout<<"            time_spent : "<<(double)(finish-start)/CLOCKS_PER_SEC<<endl<<endl;
	...
	for (int ii=0;ii<500;ii++) {
		...
		// Construct the Rd Rs Rx Ry Rz matrix: Start(To Line:506)
		...
		// if(isModelTestorNot == 1){
		// 	phy12_corr = cal_phys_corr(N_mod,Modelphy,m_1_n);
		// }
		// else{
		// 	phy12_corr = -2;
		// }
