...
double cal_Magnetic_mag_Farq_lbm_noneffsto(int N_data, int N_mod, double miu,double * deltaT,double * G,
	...
	double * Rd_2, 
	double * Rs,double * Rx,double * Ry,double * Rz,double * m_result,double lamoda0,
	...
	double * tran_r22,double * tran_r23,double * tran_r31,double * tran_r32,double * tran_r33)
{
	...
	// B part in LBM
	double * temp_lbm_B;
	temp_lbm_B = (double *)mkl_malloc( N_mod*sizeof( double ), 64 );
	#pragma omp parallel for shared(N_mod,temp_lbm_B,lamoda0,m_0_n,m_min_n,m_max_n) private(i)
	for(i=0;i<N_mod;i++)
		temp_lbm_B[i] = lamoda0*((1/(m_0_n[i]-m_min_n[i]+eps1)) + 1/(m_0_n[i]-m_max_n[i]+eps1));
	...
	for(i=0;i<N_mod;i++) {
		B[i] =  temp_lbm_B[i] + (-1)*(temp_s[i]+temp_x[i]+temp_y[i]+temp_z[i]
	...
	for(int j = 1;j<N_data;j++) {
		...
		#pragma omp parallel for shared(N_mod,temp_m,P0,W_deepw) private(i)
		for(i=0;i<N_mod;i++)
			temp_m[i] = P0[i];
	...
	// 		double * temp_d;
		...
		double * temp_lbm_AP0;
		temp_lbm_AP0 = (double *)mkl_malloc( N_mod*sizeof( double ), 64 );
		#pragma omp parallel for shared(N_mod,temp_lbm_AP0,lamoda0,m_0_n,m_min_n,m_max_n,temp_m) private(i)
		for(i=0;i<N_mod;i++)
			temp_lbm_AP0[i] = temp_m[i]*lamoda0*(1/((m_0_n[i]-m_min_n[i]+eps1)*(m_0_n[i]-m_min_n[i]+eps1)) + 1/((m_0_n[i]-m_max_n[i]+eps1)*(m_0_n[i]-m_max_n[i]+eps1)));


		#pragma omp parallel for shared(N_mod,temp_s,AP0,temp_x,temp_y,temp_z,temp_d,temp_lbm_AP0) private(i)
		for(i=0;i<N_mod;i++) {
			AP0[i] = temp_lbm_AP0[i] + (temp_s[i]+temp_x[i]+temp_y[i]+temp_z[i]
				+temp_xy[i]+temp_yx[i]+temp_xz[i]+temp_zx[i]+temp_yz[i]+temp_zy[i]
				+temp_d[i]
				+temp_dHax[i]+temp_dHay[i]+temp_dZa[i]+temp_dTa[i]
				+temp_deltaT_2[i]);
	...
	// C G :end
	double beta_lmb ;
	int num_X1_neg = 0; // number of non-negative values
	double gamma_lmb;
	gamma_lmb = 0.925;

	double temp11,temp22;
	temp11 = (m_0_n[0]-m_min_n[0])/fabs(X1[0]+eps1);
	temp22 = (m_max_n[0]-m_0_n[0])/fabs(X1[0]+eps1);
	beta_lmb = min(temp11,temp22);
	for(i=1;i<N_mod;i++) {
		temp11 = (m_0_n[i]-m_min_n[i])/fabs(X1[i]+eps1);
		temp22 = (m_max_n[i]-m_0_n[i])/fabs(X1[i]+eps1);
		beta_lmb = min(beta_lmb,temp11);
		beta_lmb = min(beta_lmb,temp22);
	}
	beta_lmb = min(beta_lmb,1.00000001);
	// cout<<" beta_lmb = "<<beta_lmb<<endl;
	#pragma omp parallel for shared(N_mod,m_result,m_0_n,X1,beta_lmb,gamma_lmb) private(i)
	for(i=0;i<N_mod;i++)
		m_result[i] = m_0_n[i] + beta_lmb*gamma_lmb*X1[i];

	double lamoda1;
	lamoda1 = (1.00000001-min(beta_lmb,gamma_lmb))*lamoda0;
	// cout<<lamoda1<<endl<<endl;
...
// calculate the data misfit function and the model objective function
int cal_datamisfit_objmodfun_lbm_noneffsto(int N_data, int N_mod,double miu,double * deltaT,
	...
	double * tran_r22,double * tran_r23,double * tran_r31,double * tran_r32,double * tran_r33)
{
	...
	for(i=0;i<N_mod;i++)
		phim_s_sum += m_alpha_s[i]*pow(temp_m[i]*Ws[i]*Ws[i]*temp_m[i] + epsilon2[i]*epsilon2[i],p2[i]/2);
	...
	// m_result : data misfit function, model objective function, total objective function
	m_result[1] = phim_s_sum+phim_x_sum+phim_y_sum+phim_z_sum+phim_xy_sum+phim_xz_sum+phim_yz_sum;

	double temp_lbm = 0; 
	for(i=0;i<N_mod;i++) {
		temp_lbm += 2*lamoda*log((m_1_n[i]-m_min_n[i])/(m_max_n[i]-m_min_n[i]+eps1)) + 2*lamoda*log((m_max_n[i] - m_1_n[i])/(m_max_n[i]-m_min_n[i]+eps1));
	}
	m_result[2] = temp_lbm;

	m_result[3] = m_result[0] + miu*m_result[1] - m_result[2];
...
double InversionMagSus_lbm_noneffsto(TESSEROID *model, OBSMAGPOS *obs_pos_mag, int N_data, int N_mod, 
	...
	int * index_lon, int * index_lat, int * index_depth, unsigned long long N_data_ULL, unsigned long long N_mod_ULL)
{
	...
	// SaveFile_double("obs_deltaTerr.dat", N_data, obs_deltaTerr);
	...
	// SaveFile_double("obs_deltaTerr_2.dat", N_data, obs_deltaTerr_2);
	...
	// Calculate the deep-weight matrix: Start
	double * W_deepw;
	...
	// SaveFile_double("W_deepw.dat", N_mod, W_deepw);

// 20141206 14:06 G=G/z
	...
	if(paraInvOrNot[0] != 0){
		...
		// SaveFile("save4.2.1_1.dat" , N_data, temp_m);
	...
	mkl_free(temp_m);
	// Rz: End
	// Construct the Rd Rs Rx Ry Rz  matrix: End
	...
	cal_datamisfit_objmodfun_lbm_noneffsto(N_data,N_mod,miu,obs_deltaT,G_T,Wd,
		...
		paraInvOrNot,
		Ws,Wx,columns_x,rowIndex_x,Wy,columns_y,rowIndex_y,Wz,columns_z,rowIndex_z,W_deepw,
		alpha,m_ref_n,m_0_n,m_min_n,m_max_n,p1,p2,px2,py2,pz2,epsilon1,epsilon2,epsilonx2,epsilony2,epsilonz2,phi_result,0,
		m_alpha_s,m_alpha_x,m_alpha_y, m_alpha_z,
		tran_r11,tran_r12,tran_r13,tran_r21,tran_r22,tran_r23,tran_r31,tran_r32,tran_r33);

	double temp_lbm = 0;
	for(i=0;i<N_mod;i++) {
		temp_lbm += (-1)*(2*log((m_0_n[i]-m_min_n[i])/(m_max_n[i]-m_min_n[i]+eps1)) + 2*log((m_max_n[i] - m_0_n[i])/(m_max_n[i]-m_min_n[i])) );
	}
	double lamoda0;
	lamoda0 = phi_result[3]/(temp_lbm+eps1);
	double lamoda1;
	cout<<"************************** ";
	cout<<"************************** "<<endl<<endl;
	cout<<"              internum : "<<internum<<endl<<endl;
	cout<<"             LB_lamoda : "<<lamoda0<<endl<<endl;
	...
	fprintf(fRstLog,"      LB_lamoda : %.12f\n\n",lamoda0);

	clock_t start,finish;
	start = clock();

	lamoda1 = cal_Magnetic_mag_Farq_lbm_noneffsto(N_data,N_mod,miu,obs_deltaT,G_T,Wd,
		obs_deltaTHax,G_THax,WdHax,obs_deltaTHay,G_THay,WdHay,obs_deltaTZa,G_TZa,WdZa,obs_deltaTTa,G_TTa,WdTa,
		obs_deltaT_2, G_T_2, Wd_2, geomag_ref_x, geomag_ref_y, geomag_ref_z,
		paraInvOrNot,Ws,Wx,
		columns_x,rowIndex_x,Wy,columns_y,rowIndex_y,Wz,columns_z,rowIndex_z,W_deepw,
		alpha,m_ref_n,m_min_n,m_max_n,m_0_n,Rd,RdHax,RdHay,RdZa,RdTa,Rd_2,Rs,Rx,Ry,Rz,m_1_n,lamoda0,
		m_alpha_s,m_alpha_x,m_alpha_y, m_alpha_z,
		tran_r11,tran_r12,tran_r13,tran_r21,tran_r22,tran_r23,tran_r31,tran_r32,tran_r33);
	...
	cal_datamisfit_objmodfun_lbm_noneffsto(N_data,N_mod,miu,obs_deltaT,G_T,Wd,
		...
		lamoda0,m_alpha_s,m_alpha_x,m_alpha_y, m_alpha_z,
		tran_r11,tran_r12,tran_r13,tran_r21,tran_r22,tran_r23,tran_r31,tran_r32,tran_r33);
	...
	fprintf(fDatamisfitTime,"num----------data_misfit--model_objective_fun---sum_datamisfit_mdf----------------Time------------------miu----------phy12_corr\n");
	fprintf(fDatamisfitTime,"%d %20.12f %20.12f %20.12f %20.12f %20.12f %20.12f\n",internum,phi_result[0],phi_result[1],
		phi_result[3],(double)(finish-start)/CLOCKS_PER_SEC,log10(miu),phy12_corr);
	fclose(fDatamisfitTime);

	cout<<"           data_misfit : "<<phi_result[0]<<endl<<endl;
	cout<<"           modelobjfun : "<<phi_result[1]<<endl<<endl;
	cout<<"            sum_objfun : "<<phi_result[3]<<endl<<endl;
	...
	for (int ii=0;ii<500;ii++) {
		...
		double phi_derr;
		phi_derr = fabs(phi_result[3] - phi_dtemp)/phi_result[3];
		double phi_log;
		phi_log = fabs(phi_result[2]/phi_result[3]);
		...
		for(i=0;i<N_mod;i++)
			m_bianhua_n[i] = fabs(m_1_n[i] - m_0_n[i]);
